...
class GameManager_GUI:
    def __init__(self):
        ...
        self.root.resizable(width=False, height=False)  #창 크기 변경 불가
    ...
    def draw_board(self):
        ...
        for i, v in enumerate(self.board):
            if v == ".":
                ...
                self.canvas.create_image(x,y,anchor="row",image=self.images["O"])
            elif v == "X":
                self.canvas.create_image(x,y,anchor="row",image=self.images["X"])
            x+=self.TILE_SIZE
            if i % 3 == 2:
                x=0
                y+=self.TILE_SIZE
